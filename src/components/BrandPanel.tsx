...
export default function BrandPanel() {
  return (
    <div className="hidden lg:flex flex-1 flex-col justify-between p-12 text-white relative overflow-hidden bg-[#0A0B14]">
      ...
      <div className="absolute bottom-[-10%] right-[-10%] w-[400px] h-[400px] bg-amber-600/10 rounded-full blur-[100px] pointer-events-none" />

      <div className="relative z-10 space-y-12">
        <div className="flex items-center gap-3">
          <MascotLogo className="w-10 h-10 transform scale-125" />
          <span className="text-2xl font-black tracking-tighter">RelatórioFlow<span className="text-orange-500">.</span></span>
        </div>
        ...
        <div className="space-y-6">
          {[
            ...
            <div key={i} className="flex items-start gap-4 group">
              <div className="h-10 w-10 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center shrink-0 mt-0.5 group-hover:border-orange-500/50 transition-colors">
                {f.icon}
              </div>
      ...
      <div className="relative z-10 mt-8 pt-8 border-t border-white/5 max-w-md">
        ...
        <div className="flex items-center gap-3 mt-4">
          <div className="h-9 w-9 rounded-full bg-gradient-to-br from-orange-500 to-amber-600 flex items-center justify-center text-xs font-bold shadow-lg">
            EG
          </div>
          <div>
      ...
      <div className="absolute inset-0 pointer-events-none opacity-20">
        <div className="absolute top-[20%] left-[30%] w-2 h-2 bg-white rounded-full animate-pulse shadow-[0_0_10px_2px_rgba(249,115,22,0.3)]" />
        <div className="absolute bottom-[40%] left-[10%] w-1.5 h-1.5 bg-orange-500 rounded-full animate-ping" />
      </div>
    </div>
  );
}